...
exports.createLaptop = async (req, res) => {
  try {
    ...
    const laptop = await models.Laptop.create({
      item_id:item.id,
      processor,
      ram,
      storage_type,
     ...
     const qrData = process.env.API_URL+"/api/laptops/"+laptop.id;

     const qrImagePath = path.join(__dirname, "../public/qrcodes", `item_${laptop.id}.png`);
     ...
     item.asset_tag = `/qrcodes/item_${laptop.id}.png`;
     await item.save();
...
exports.getAllLaptops = async (req, res) => {
  try {
    const laptops = await models.Laptop.findAll({ order: [["id", "DESC"]], include: [
        {
          model: models.Item,
          // as: "item", // use this if you set `as: "item"` in Laptop.associate
        },
      ], });
...
exports.getLaptopById = async (req, res) => {
  try {
    const { id } = req.params;
    const laptop = await models.Laptop.findByPk(id, {
        include: [
          {
            model: models.Item,
            // as: "item", // use this if you set `as: "item"` in Laptop.associate
...
exports.updateLaptop = async (req, res) => {
  try {
    const { id } = req.params;
    const laptop = await models.Laptop.findByPk(id);
    const item = await models.Item.findByPk(laptop.item_id)
